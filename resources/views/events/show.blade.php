    ...
    <main class="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-3 gap-12 text-slate-900">
        <div class="lg:col-span-1">
            <div class="sticky top-32">
                ...
                @else
                    <div class="w-full aspect-[3/4] bg-indigo-50 rounded-[2.5rem] flex items-center justify-center">
                ...
                @endif
                <div class="mt-6 p-5 bg-white rounded-3xl border border-slate-100 shadow-sm">
                    <div class="flex justify-between items-center">
                        <span class="text-slate-500 font-medium text-sm">Sisa Tiket</span>
                        <span class="font-black text-indigo-600">{{ $event->stock }} tiket</span>
                    </div>
                    <div class="mt-3 w-full bg-slate-100 rounded-full h-2">
                        <div class="bg-indigo-600 h-2 rounded-full" style="width: {{ min(100, $event->stock) }}%"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-12">
            <div class="space-y-4">
                @if($event->category)
                    <a href="{{ route('category.show', $event->category->slug) }}"
                       class="inline-block px-4 py-1.5 bg-indigo-100 text-indigo-700 rounded-full text-sm font-bold uppercase tracking-wider hover:bg-indigo-200 transition">
                        {{ $event->category->name }}
                    </a>
                @endif
